 ...
 *
 * 升级说明：
 * - 支持ID引用：categories/location字段存储的是system_config的_id
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // 处理和格式化数据
    const items = aggregateResult.list.map(item => {
      ...
      if (item.priceUnitDetails && item.priceUnitDetails.length > 0) {
        ...
        priceUnitId = item.priceUnitDetails[0]._id;
      }
